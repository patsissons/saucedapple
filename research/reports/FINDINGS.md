...
One technique changes the game: **r.jina.ai, called from the user's browser.**
Bolting it on as a fallback after the current extractor takes measured coverage
from **50% → 86%** overall and, crucially, **class-C hard paywalls from 0% →
67%** (FT and Economist become fully readable; only WSJ resists). It costs the
Worker **zero CPU** and fetches from the **user's residential IP**, sidestepping
both the 10 ms CPU budget and the Cloudflare-IP publisher blocks in one move.

Everything else is either a cheap enhancement (a JSON-LD CPU fast-path, on-device
AI summaries, a structured "read elsewhere" list) or a dead end worth recording
so it isn't re-litigated. Apple has no free content route, and no free technique
can serve News+ exclusives or crack WSJ.
...
| 3 | **r.jina.ai reader** | **Browser** | most of B, FT + Economist (C) | **0 Worker CPU**, residential IP, seconds | **P11, P4** |
...
### Coverage the ladder buys (measured)

| Class | Today (rung 1–2) | + rung 3 (jina) |
| --- | --- | --- |
| A — open | 80% | **100%** |
| B — soft/metered | 50% | **83%** |
| C — hard/bot | 0% | **67%** (FT+Econ; WSJ still 0) |
| **All** | **50%** | **86%** |
...
| --- | --- | --- | --- |
| 1 | **r.jina.ai (client-side)** — P11 | **SHIP** | 0→67% class C, 50→86% overall; zero Worker cost; residential IP |
...
| 8 | **Browser Rendering** — P9 | **PARK** | inherits the Cloudflare-IP block; dominated by jina; 10 min/day cap, unmeterable |
...
## Ship-first roadmap (three PR-sized changes)

1. **Rung 3 — jina, client-side.** In `reader-view.tsx`, when `/api/extract`
   fails or returns below `MIN_TEXT_LENGTH`, `fetch(\`https://r.jina.ai/\${canonicalUrl}\`)`
   from the SPA, render the returned markdown through the existing
   `sanitizeArticleHtml()` path. Add a progress state. Server change is nil (the
   canonical URL is already in the resolve payload). Guard on failure/timeout so
   it degrades to rung 4.
2. **Rung 1 — JSON-LD fast-path.** Add `extractJsonLdArticle()` beside
...
3. **Rung 4 — structured "read elsewhere".** Server route that queries Google
...
- **WSJ specifically resists everything free** — server fetch (401), Wayback
  (JS shell), jina (23-word stub), Browser Rendering (Cloudflare IP → 401),
  archive.today (automation-blocked). The archive.today *link* is the only path,
  and only a human clicking it. This is expected, not a bug (AGENTS.md).
- **The 10 ms CPU budget is already exceeded** by today's Readability parse
  (median 31 ms, p90 143 ms on the dev laptop). Keep new CPU off the Worker —
  which is exactly why the winning rung runs in the browser. Confirm real-Worker
  CPU headroom before adding any server-side parsing.
...
- **Everything is laptop/residential evidence.** jina's client-side path means
  the laptop-vs-Worker IP trap doesn't apply to the #1 result (the browser is
  the residential client in production too) — but any *server-side* rung should
  still be confirmed from a deployed Worker before shipping.
- **jina is a third-party dependency** (free ~20 rpm, no key today). Ship it as a
  graceful, user-initiated rung with a fallback, not a hard dependency.
